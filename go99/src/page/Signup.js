  ...
  return (
    <React.Fragment>
      <Grid padding="104px 20px 0 20px">
        <Text size="36px" color="#455154" bold margin="0px 0px 32px 16px">
          회원가입
        </Text>
        <Input
        ...
        <Grid hide={pwd === "" ? "none" : null}>
          <Text
            color={
              checkPassword(pwd) && pwd.includes(id) === false
                ? "#1fc40f"
                : "#ff5d5d"
            }
            bold
            margin="-30px 0px 13px 5px"
            size="0.75rem"
          >
            {checkPassword(pwd) && pwd.includes(id) === false
              ? "사용할 수 있는 비밀번호입니다"
              : "아이디를 포함하지 않은 영문자+숫자 4자리이상이 필요합니다"}
          </Text>
        </Grid>
        <Input
          type="password"
        ...
        <Button padding="16px" fontSize="16px" _onClick={singUp}>
          ToDo99 회원가입
        </Button>
      </Grid>
    </React.Fragment>
